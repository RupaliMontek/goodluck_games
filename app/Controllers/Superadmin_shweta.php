    ...
    else
{
    ...
    elseif ($mode == "jackpot_2x") 
    {
    // Save current mode and values
    ...
    // Save the previous mode and data for resetting after 1 minute
    $this->session->set('previous_mode', $previous_mode);
    $this->session->set('previous_data', $previous_data);
    
    // Set a timer to reset the previous mode after 1 minute
    // This will need to be done through a cron job or a scheduled task in PHP.
}
...
$notification_status = $this->SuperAdminModel->number_insert($data_insert);
...
{
    // This method should be scheduled to run after 1 minute

    // Retrieve previous mode values
    $previous_mode = $this->session->userdata('previous_mode');
    $previous_values = $this->SuperAdminModel->get_values_by_mode($previous_mode);
    
    if ($previous_values) {
        $this->SuperAdminModel->number_insert($previous_values);
    }
}
