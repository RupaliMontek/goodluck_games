...
*/
public function saveStoppedNumber() {
    $user_id = $this->request->getPost('user_id'); // Change here
    $mode_set = $this->request->getPost('mode_set'); // Change here
    $remainingNumbers = $this->request->getPost('remainingNumbers'); // Change here
    $target_value = $this->request->getPost('target_value'); // Change here
 ...
 $now = Time::now();
 $spinnerStoppedNumberModel = new SpinnerStoppedNumber();
        $result = $spinnerStoppedNumberModel->saveStoppedNumber_new($user_id, $target_value, $mode_set, $now);

    if ($result) {
        return $this->response->setJSON(['success' => true, 'value' => $target_value]);
    } else {
        return $this->response->setJSON(['success' => false, 'message' => 'Database insert failed']);
    }
}
