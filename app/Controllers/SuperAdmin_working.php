    ...
    else
{
    ...
    if($mode=='jackpot' || $mode=='next' || $mode=='high' || $mode=='low' || $mode=='mediam' || $mode=='jackpot_2x')
    {
        $data_insert = array(
        'low'          =>    @$low,
...
//print_r($post);exit;
    if (!empty($post)) {
        ...
        $data = array();
$mode=$post["mode"];
        // Handle the 'jackpot_2x' mode first
        if ($post["mode"] == 'jackpot_2x') {
            $number_array = [
                @$post["S2number1"], @$post["S2number2"], @$post["S2number3"], @$post["S2number4"],
                @$post["S2number5"], @$post["S2number6"], @$post["S2number7"], @$post["S2number8"],
                @$post["S2number9"], @$post["S2number0"]
            ];

            $jackpot_2x = array_filter($number_array, 'strlen'); // Remove empty elements
            $jackpot_2x = implode(",", $jackpot_2x);
            $data["numbers"] = $jackpot_2x;
              // Save current mode and numbers
            $this->session->set('mode', 'jackpot_2x');
            $this->session->set('previous_data', $jackpot_2x);
        }
        ...
        if ($post["mode"] !== "next") {
            ...
            $S1number_array = [
                @$post["number_table_val0"], @$post["number_table_val1"], @$post["number_table_val2"], @$post["number_table_val3"],
                @$post["number_table_val4"], @$post["number_table_val5"], @$post["number_table_val6"], @$post["number_table_val7"],
                @$post["number_table_val8"], @$post["number_table_val9"]
            ];

            $playing_user = [
                @$post["number_0_playing_user"], @$post["number_1_playing_user"], @$post["number_2_playing_user"], @$post["number_3_playing_user"],
                @$post["number_4_playing_user"], @$post["number_5_playing_user"], @$post["number_6_playing_user"], @$post["number_7_playing_user"],
                @$post["number_8_playing_user"], @$post["number_9_playing_user"]
            ];

            // Mode-specific logic
            if (@$post["mode"] == "mediam") {
                sort($playing_user);
                $median_positions = array(floor(count($playing_user) / 2));
                $number_array = [$playing_user[$median_positions[0]]];
            } elseif (@$post["mode"] == "low") {
                sort($playing_user);
                $number_array = array_slice($playing_user, 0, 4);
            } elseif (@$post["mode"] == "high") {
            ...
            foreach ($number_array as $num) {
                foreach ($playing_user as $key => $user) {
                    if ($num == $user) {
                        $datas[] = $S1number_array[$key];
                    }
                }
            }
             $previous_mode = $this->session->get('mode');
            $previous_data = $this->session->get('previous_data');
        }

        // Check latest entry for 'jackpot_2x'
        $latest_entry_state = $this->NumbersModel->get_first_2x_jackpot();

        if ($post["mode"] == 'jackpot_2x') {
            // Get second last jackpot record
            $data_insert_second_last =[
            'low' => @$low,
            'intermediate' => @$intermediate,
            'high' => @$high,
            'jackpot' => @$jackpot,
            ...
            // Insert second last record again
            $this->NumbersModel->number_insert($data_insert_second_last);
            $this->session->set('mode', $mode);
        }
  // Save mode and data
        $this->session->set('mode', $post["mode"]);
        $this->session->set('previous_data', $data["numbers"]);
        // Insert new record for any mode (including jackpot_2x)
   ...
   if(!empty($post))
    {
        ...
        if ($post["mode"] == 'jackpot_2x') {
            ...
            $jackpot_2x = array_filter($number_array, 'strlen'); // Remove empty elements
            $jackpot_2x = implode(",", $jackpot_2x); // Convert array to a comma-separated string
            $data["numbers"] = $jackpot_2x;
        }
        if($post["mode"]=="next")
{
