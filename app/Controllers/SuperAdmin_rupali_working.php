...
//exit;
    if (!empty($post)) {
        $number_array = $this->process_numbers($post);
        $mode = $post["mode"];
...
private function process_numbers($post)
{
    // print_r($post);
    $number_array = [];
    if (isset($post["mode"]) && $post["mode"] === 'jackpot_2x') {
        // Process numbers for jackpot_2x mode
        for ($i = 0; $i <= 9; $i++) {
            // If the value exists, add it to the array, otherwise add null
            $number_array[] = isset($post["S2number" . $i]) && $post["S2number" . $i] !== '' ? $post["S2number" . $i] : NULL;
        }
    } else if(isset($post["mode"]) && $post["mode"] === 'next'){
        // Process numbers for other modes
        for ($i = 1; $i <= 9; $i++) {
            // If the value exists, add it to the array, otherwise add null
            $number_array[] = isset($post["S1number" . $i]) && $post["S1number" . $i] !== '' ? $post["S1number" . $i] : NULL;
        }
      }
        else {
        // Process numbers for other modes
        for ($i = 1; $i <= 9; $i++) {
            // If the value exists, add it to the array, otherwise add null
            $number_array[] = isset($post["number_table_val" . $i]) && $post["number_table_val" . $i] !== '' ? $post["number_table_val" . $i] : NULL;
        }
    }
...
private function prepare_data_for_insertion($number_array, $mode)
{
    ...
    switch ($mode) {
        case 'high':
            $data_insert['high'] = implode(",", array_slice($number_array, 0, 3));
            break;
        case 'low':
            $data_insert['low'] = implode(",", array_slice($number_array, 0, 4));
            break;
        case 'mediam':
            $data_insert['intermediate'] = implode(",", array_slice($number_array, 0, 2)); // Example logic
            break;
        case 'jackpot':
            
             // Filter out null values and then implode
            $filtered_numbers = array_filter($number_array, function($num) {
                return $num !== NULL;
            });
            $data_insert['jackpot'] = implode(",", $filtered_numbers);
            break;
        ...
        case 'jackpot_2x':
            ...
            $data_insert['jackpot_2x'] = implode(",", $filtered_numbers);
            break;
        case 'next':
             // Filter out null values and then implode
            $filtered_numbers = array_filter($number_array, function($num) {
