...
<!--shweta sound change end-->
        ...
        <div class="row">
            <div class="col-sm-12 col-md-12 col-lg-12 frusernameHedddd">
                 <h2 class="usernameee"><i id="soundIcon" class="fas fa-volume-up" style="font-size: 18px;"></i><i class="fa fa-user"></i> : <?=  ucwords($username); ?></h2>
                <div class="forbettt"> 
         ...
         <!-- Audio Element -->
    <audio id="tickSound" src="<?= base_url('frontend/game_audio/clock-ticking-2.wav'); ?>" preload="auto" loop></audio>
...
<div id="testPopup">
        <p>This is a test popup. You are about to leave the page.</p>
        <button id="closePopup">Close Popup</button>
    </div>
<!--shweta sound change start-->
<!-- HTML Audio Elements -->
    <audio id="button-sound" src="<?php echo base_url("frontend/game_audio/click558.mp3"); ?>" preload="auto" autoplay></audio>
...
<script>
  $(window).on('beforeunload', function (event) {
            ...
            // URL to the logout endpoint
            

            // Prepare the AJAX request
            $.ajax({
                url: base_url + 'login/auto_logout',
                type: 'POST',
                data: JSON.stringify({ user_id: 151 }),
                async: false, // Ensure the request completes before the page unloads
                //data: { logout: true }, // Example data to send with the request
                success: function (response) {
                    $('#testPopup').show();
                    console.log('Logout request sent successfully.');
                },
                error: function (jqXHR, textStatus, errorThrown) {
                    console.error('Error sending logout request:', textStatus, errorThrown);
                }
            });
