...
<link rel="manifest" href="<?php echo base_url("frontend/images/site.webmanifest");?>">
  ...
  <style>
     /* CSS for disabled buttons */
        ...
        /* Ensure the button has no default styling */
#toggle-audio {
...
#toggle-audio i {
    ...
    color: #ffc107; /* Set icon color (adjust as needed) */
}
       ...
       <ul class="horizontal-list" id="last_10_reuslts">
        <?php
            ...
            foreach ($last_ten_results as $result): 
                if($result["mode_set"]=='jackpot_2x' || $result["mode_set"] =='jackpot')
                {
                ...
                if ($count < 10): 
        ?>
            <li class="<?= $jackpot_class; ?>"><?php if($result["mode_set"]=='jackpot_2x' || $result["mode_set"] =='jackpot')
                {
                     echo '';
                     $jackpot_class = "jackpot2_class";
...
// shweta sound change start
            // JavaScript Code
// const incrementSpeed = 500; // Define your increment speed in milliseconds
...
for (let i = 0; i < 10; i++) {
    (function(i) {
        $(`#myButton${i}`).on('mousedown', function() {
            ...
            playSound(); // Play sound on initial click
            
            longPressIntervalId = setInterval(() => {
                incrementValue(i);
                playSound(); // Play sound on each increment during long press
            }, incrementSpeed);
...
// Example incrementValue function
function incrementValue(index) {
    // Your increment logic here
    console.log(`Button ${index} incremented`);
}
// shweta sound change end
            let selectedButtonIndex;
...
function spinner_spin() {
    ...
    spinnerCalled = true; // Set the flag to true
...
// Shweta sound end
    ...
    $.ajax({
        ...
        success: function(data) {
            ...
            let spinDuration = 3000; // Increased duration for slower spin (3 seconds)
            let intervalDuration = 20; // Decreased interval for smoother progress
...
if ((mode_set === 'jackpot_2x') && winnerValue !== 0) {
    // Highlight the winning button with green
    for (let i = 0; i <= 9; i++) {
        if (/* condition to identify the winning index */) {
            // Assuming you have a way to determine which index won
            let winningButtonId = 'showNobtn' + i;
            let winningButton = document.getElementById(winningButtonId);
            if (winningButton) {
                winningButton.classList.add('highlight-green'); // Add a class for green highlight
            }
        }
    }
    ...
    // Deactivate active buttons inside .forBottomCounterr
    $('.forBottomCounterr button.active').each(function() {
        $(this).removeClass('active');
    });
}
                    ...
                    // Reset spinner position after 5 seconds
                    // Remove or adjust this part if not needed
                    // setTimeout(() => {
                    //     myChart.options.rotation = 0;
                    //     myChart.update();
                    // }, 5000);
...
function get_last_10_win_numbers()
{
   ...
   ({
        ...
        success: function(response)
        {
            ...
            {
                var item = data[i];
                if(item.mode_set=='jackpot_2x' || item.mode_set=='jackpot')
                {
                    var jackpot_class = "jackpot2_class";
                }
                else
                {
                    var jackpot_class = "";
                }  
                // console.log(jackpot_class);
                var item = data[i];
                var jackpot_class = "<?php echo $jackpot_class; ?>";  // PHP variable used in JavaScript
                var listItem = $('<li class="' + jackpot_class + '">' +
                '' +
                item.numbers + 
                '</li>');
