            ...
            let rotationInterval = window.setInterval(() => {
                ...
                if (step >= totalSteps) {
                    ...
                    if (showNobtnElement) 
                    {
                        ...
                        // shweta new end
                                ///////////sayali-----------commented by shweta
                            //      $("#betok").hide();
                            //     $("#prevok").show();
                            //   bet_previous();
                        // shweta changes end
                        
                        //////////////////////////////////////////////////////////////////need to cmt for check
...
{   
    ...
    $('#totalamt,#betok,#betcancel,#onebetcancel,#myButton1,#myButton2,#myButton3,#myButton4,#myButton5,#myButton6,#myButton7,#myButton8,#myButton9,#myButton0,#logoutBtn').prop('disabled', true).addClass('disabled-button');
    var spin_btn = document.getElementById("spin-btn");
    spin_btn.disabled = true;
...
$(document).ready(function() {
    ...
    function initializeGameAmount() {
  // Get the game amount from localStorage
  let gameAmount = localStorage.getItem('totalamt');
  let storedMainScore = localStorage.getItem('main_score');
 // let storedMainScore = parseInt($("#main_score").text());
console.log(storedMainScore);
//  let mainscore=localStorage.getItem('main_score');

  // If no game amount is stored, default to 100
//   if (gameAmount === null) {
//     gameAmount = 100; // or any default value you prefer
//   }

  // Display the game amount
  document.getElementById('totalamt').textContent = gameAmount;
  document.getElementById('main_score').textContent = storedMainScore;
}

// Function to update the game amount and store it in localStorage
function updateGameAmount(newTotalAmt, newMainScore) {
  // Update the content of the elements on the page
  document.getElementById('totalamt').textContent = newTotalAmt;
  document.getElementById('main_score').textContent = newMainScore;

  // Store the new values in localStorage
  localStorage.setItem('totalamt', newTotalAmt);
  localStorage.setItem('main_score', newMainScore);
}

// On page load, initialize the game amount
window.onload = function() {
  initializeGameAmount();
}
...
<script>
  // This flag indicates whether the page was reloaded or not
  sessionStorage.setItem('isPageReload', 'false');

  // Function to handle auto-logout on tab close or browser close
  function handleAutoLogout(event) {
    // Check if the page was not reloaded (i.e., a tab/browser close)
    if (sessionStorage.getItem('isPageReload') === 'false') {
      // If it's not a reload, send the logout request
      var logoutUrl = base_url + 'login/auto_logout';
      navigator.sendBeacon(logoutUrl, JSON.stringify({ user_id: 123 }));

      // Clear the pageLoaded flag in localStorage
      localStorage.removeItem('pageLoaded');
    }

    // Optional: Provide a confirmation message
    event.preventDefault();
    event.returnValue = ''; // Trigger a confirmation dialog in some browsers

    return ''; // Some browsers may require a return value to show the dialog
  }

  // Detect if the page is being reloaded
  window.addEventListener('beforeunload', function (event) {
    // Set the reload flag in sessionStorage to true on page reload
    sessionStorage.setItem('isPageReload', 'true');
  });

  // Handle tab/browser close scenario using the unload event
  window.addEventListener('unload', function () {
    if (sessionStorage.getItem('isPageReload') === 'false') {
      // If the page was not reloaded, call the logout function
      handleAutoLogout();
    }
  });

  // Reset the reload flag if the page is successfully loaded
  window.addEventListener('pageshow', function (event) {
    if (event.persisted || performance.navigation.type === performance.navigation.TYPE_RELOAD) {
      // If the page was reloaded, set the reload flag to true
      sessionStorage.setItem('isPageReload', 'true');
    } else {
      // Otherwise, set the reload flag to false (indicating a fresh load)
      sessionStorage.setItem('isPageReload', 'false');
    }
  });
</script>
