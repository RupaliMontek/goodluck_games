...
function spinner_spin() {
    ...
    // First AJAX call to get numbers based on mode
    $.ajax({
        ...
        success: function (data) {
            try {
                ...
                console.log('Server Response:', parsedObject);
                ...
                // Check for undefined targetValue
                if (typeof targetValue === 'undefined') {
                    console.warn('Target value is undefined. Please check server response.');
                    return;
                }
                ...
                let targetAngle = getTargetAngle(targetValue); // Call a helper function to get the angle

                if (typeof targetAngle === 'undefined') {
                    console.error(`Target value ${targetValue} not found in rotation values. Refreshing spinner settings.`);
                    return;
                }

                startSpinnerAnimation(targetAngle, targetValue, mode_set);
            } catch (e) {
                console.error('Error parsing data from getNumbersBasedOnMode:', e);
                alert('Failed to retrieve numbers for the spin. Please try again.');
            }
        },
        error: function (jqXHR, textStatus, errorThrown) {
            console.error('Error fetching numbers based on mode:', textStatus, errorThrown, jqXHR.responseText);
            alert('Failed to retrieve numbers for the spin. Please try again.');
        }
    });
...
// Helper function to get the target angle
function getTargetAngle(targetValue) {
    for (let i of rotationValues) {
        if (i.value === targetValue) {
            return (i.minDegree + i.maxDegree) / 2;
        }
    }
    return undefined;
}

// Function to save the stopped number
function saveStoppedNumber(targetValue, mode_set, remainingNumbers) {
    $.ajax({
        url: base_url + 'user/saveStoppedNumber',
        type: 'POST',
        data: { 
            user_id: '<?php echo $_SESSION["user_id"];?>', // Replace with the actual user ID
            mode_set: mode_set,
            remainingNumbers: remainingNumbers.join(","), // Send as comma-separated values
            target_value: targetValue // Add target value to the request
        },
        dataType: 'json', // Automatically parse response as JSON
        success: function (responseData) {
            if (responseData.success) {
                console.log('Stopped number for all users: ', responseData.value);
                console.log('Expected Target Value: ', targetValue); // Debug log for expected value
            } else {
                console.error('Failed to retrieve/save number:', responseData.message);
                alert('Failed to retrieve/save number: ' + responseData.message);
            }
        },
        error: function (jqXHR, textStatus, errorThrown) {
            console.error('Error retrieving/saving number:', textStatus, errorThrown, jqXHR.responseText);
            alert('Failed to retrieve/save number. Please try again.');
        }
    });
}

// Function to start spinner animation
function startSpinnerAnimation(targetAngle, targetValue, mode_set) {
    ...
    let rotationInterval = window.setInterval(() => {
        ...
        if (currentTime >= endTime) {
            ...
            // AJAX call to save the stopped number
            // Pass remainingNumbers here as it is required in the save function
            let remainingNumbers = [] // Assuming you already have this array defined from the server response or another source
            saveStoppedNumber(targetValue, mode_set, remainingNumbers); // Pass mode_set and remainingNumbers to the save function
...
function spinner_spinnot_working() {
    ...
    // Capture numbers and prepare them for backend (if necessary)
    let showNobtn1 = parseInt(document.getElementById('showNobtn1').innerText);
    $("#number1").val(showNobtn1);
    let number1 = $("#number1").val();
    ...
    // AJAX call to get globally synchronized spin result based on mode_set
    $.ajax({
        url: base_url + 'user/getGlobalSpinResult',
        type: "GET",
       // data: { mode_set: mode_set }, // Pass mode_set to the backend
        success: function (data) {
            let parsedObject = JSON.parse(data);
            let targetValue = parseInt(parsedObject.targetValue);  // This targetValue should be the same for all users
            let mode_set = parsedObject.mode;

            // Use the targetValue for the spinner
            let targetAngle;
            for (let i of rotationValues) {
                if (i.value === targetValue) {
                    targetAngle = (i.minDegree + i.maxDegree) / 2;
            ...
            let rotationInterval = window.setInterval(() => {
                ...
                if (currentTime >= endTime) {
                    ...
                    $.ajax({
                        url: base_url + 'user/saveStoppedNumber',
                        type: 'POST',
                        data: { stoppedNumber: targetValue, mode_set: mode_set }, // Send the same targetValue to save
                        success: function(response) {
