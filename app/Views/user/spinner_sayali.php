...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function storeNumbers() {
        for (let i = 0; i <= 9; i++) {
            let inputElement = document.getElementById(`number${i}`);
            if (inputElement) {
    ...
    // Function to retrieve values from localStorage and set to hidden inputs
        ...
        for (let i = 0; i <= 9; i++) {
            ...
            if (inputElement) {
                if (storedValue !== null) {
                    ...
                    inputElement.value = 0; // Set default value if nothing is stored
                }
            }
        }
    }

    // Call retrieveNumbers to set values on page load
    retrieveNumbers();

    // Example of setting values and storing them (call this function as needed)
    function exampleSetValues() {
        for (let i = 0; i <= 9; i++) {
            let inputElement = document.getElementById(`number${i}`);
            if (inputElement) {
                // Example: Increment values or set them as needed
                inputElement.value = (parseInt(inputElement.value) || 0) + 1;
            }
        }
        storeNumbers(); // Save to localStorage after setting values
    }

    // Example call (you can replace this with your own logic to set values)
    exampleSetValues();
});
...
<script>
(function() {
    const AUTO_LOGOUT_URL = base_url + 'login/auto_logout';
    const REFRESH_FLAG_KEY = 'isRefresh';

    function handleLogout() {
        var userId = getUserID(); // Function to retrieve the current user ID
        fetch(AUTO_LOGOUT_URL, {
            method: 'POST',
            headers: {
    ...
    function getUserID() {
        return 123; // Replace with your actual logic to get the user ID
    }

    // On page load
    window.addEventListener('load', function() {
        // Clear the refresh flag on load
    ...
    window.addEventListener('pageshow', function(event) {
        if (event.persisted) {
            // The page was loaded from cache, so it’s a refresh
    ...
    // On page unload (tab close, browser close)
    window.addEventListener('beforeunload', function(event) {
        const isRefresh = localStorage.getItem(REFRESH_FLAG_KEY) === 'true';

        if (!isRefresh) {
            // If it's not a refresh, handle the logout
            handleLogout();
        }
    });
})();
