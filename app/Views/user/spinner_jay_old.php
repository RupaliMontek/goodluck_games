...
function get_last_10_win_numbers() {
    $.ajax({
        ...
        type: 'POST',
        data: {
            user_id: userId // Pass the user ID if required
        },
        success: function(response) {
            var data = JSON.parse(response);
            $('#last_10_results').empty(); // Corrected the typo in the ID
            ...
            for (var i = 0; i < Math.min(10, data.length); i++) {
                ...
                $('#last_10_results').append(listItem); // Corrected the typo in the ID
            }
        },
        error: function(xhr, status, error) {
