...
$('#onebetcancel').on('click', function(){
    ...
    setInterval(get_as_per_mode_array_numbers_array, 20);
    get_user_balance_amount();
...
function spinner_spin() {
    ...
    $.ajax({
        ...
        success: function(data) {
            ...
            let totalamt = parseInt(document.getElementById('totalamt').innerText);
            let winval = parseInt(document.getElementById('winnerBtn').innerText);
            ...
            // Populate all number inputs
            for (let i = 0; i <= 9; i++) {
                $(`#number${i}`).val(parseInt(document.getElementById(`showNobtn${i}`).innerText));
            }
            ...
            // Mapping targetValue to the resultValue
            let resultValue;
            switch (targetValue) {
            ...
            let rotationInterval = window.setInterval(() => {
                ...
                if (myChart.options.rotation >= 360) {
                    myChart.options.rotation = 0; // Reset rotation to keep it within bounds
                }

                if (count > 15 && myChart.options.rotation >= targetAngle) {
                    ...
                    const showNobtnId = 'number' + targetValue;
                    const showNobtnElement = $("#" + showNobtnId).val(); 
                    if (showNobtnElement) {
                        let winnerValue = 0;
                        const showNobtnValue = parseInt(showNobtnElement.trim());
                        if (!isNaN(showNobtnValue)) {
                            winnerValue = mode_set !== 'jackpot_2x' && mode_set !== 'jackpot' ? showNobtnValue * 9 : showNobtnValue * 18;
                        ...
                        if (winnerValue !== 0) {
                            document.getElementById("winnerBtn").classList.add("btn", "glow"); 
                            document.getElementById("take-btn").classList.add("btn", "glow"); 
