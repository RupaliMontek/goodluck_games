        ...
        <div class="row">
            <div class="col-sm-12 col-md-12 col-lg-12">
              ...
              <div class="forBottomCounterr">
                    ...
                    <button  onclick="incrementValue('4')" id="myButton4" type="button" class="btn">4</button>
                    </div>
                    <div class="CounterrOuterrbtn">
                    ...
                    <button  onclick="incrementValue('6')"  id="myButton6" type="button" class="btn">6</button>
                    </div>
                    <div class="CounterrOuterrbtn">
                        <span  class="showNobtn" id="showNobtn7"></span>
                    <button  id="myButton7" type="button" onclick="incrementValue('7')"  class="btn">7</button>
                    </div>
                    <div class="CounterrOuterrbtn">
...
<div class="row">
            <div class="col-sm-12 col-md-12 col-lg-12">
                <div class="forBottompartt">
                    ...
                    <button type="button" class="btn" id="totalamt" value=""></button>
                    </div>
...
$(document).ready(function() {
    ...
    $('#betok').on('click', function() {
        // Disable specific buttons individually
        $('#myButton1, #myButton2, #myButton3, #myButton4, #myButton5, #myButton6, #myButton7, #myButton8, #myButton9, #myButton0').prop('disabled', true);
    });
});
...
var showNobtnvalue = document.getElementById('showNobtnvalue');
    showNobtnvalue.value = number;
...
// Array to store the IDs of the elements
var elementIds = [
...
// Loop through each element ID and calculate the total
...
if (totalAmountElement) {
    totalAmountElement.innerText = totalAmt;
}
}
const calculateWinnerAmount = (stoppedNumber) => {
    const stoppedNumberValue = parseInt(document.getElementById('showNobtn' + stoppedNumber).innerText);

    const winnerAmount = stoppedNumberValue * 9;

    // Return the winner's amount
    return winnerAmount;
};

const updateWinnerBox = (amount) => {
    const winnerBox = document.getElementById('winnerBtn');

    winnerBox.innerText = amount;
};


</script>

<!-- for loader code start here -->
...
<!-- for loader code end here -->
<script>
    $(document).ready(function() {
  // Function to generate gradient colors
  function generateGradientColors(count) {
    const colors = [];
    const colorStops = count; // Number of color stops
    
    // Define the start and end colors (e.g., red to yellow)
    const startColor = [255, 0, 0]; // Red
    const endColor = [255, 255, 0]; // Yellow
    
    // Calculate the step increment for each color channel
    const step = [
      (endColor[0] - startColor[0]) / colorStops,
      (endColor[1] - startColor[1]) / colorStops,
      (endColor[2] - startColor[2]) / colorStops
    ];

    // Generate gradient colors based on the number of stops needed
    for (let i = 0; i < colorStops; i++) {
      const r = Math.round(startColor[0] + step[0] * i);
      const g = Math.round(startColor[1] + step[1] * i);
      const b = Math.round(startColor[2] + step[2] * i);
  ...
  // Number of gradient colors needed
  const numberOfColors = 10; // Example: 10 colors

  // Generate gradient colors
  const gradientColors = generateGradientColors(numberOfColors);

  // Output the gradient colors in the console
  console.log('Generated Gradient Colors:', gradientColors);

  // Example usage: Apply gradient colors to DOM elements
  $('.color-box').each(function(index) {
    // Set background color for each box based on the generated gradient colors
    if (index < gradientColors.length) {
      $(this).css('background-color', gradientColors[index]);
    }
  });
});
