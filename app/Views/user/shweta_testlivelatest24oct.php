...
// }
//   shweta added exit 19th september
            ...
            function updateTotalAmount() 
{
    ...
    //console.log(totalamt);
            ...
            //////////////commented by shweta 19 sep
            
            // Generate the buttons and spans dynamically
            // for (let i = 0; i < 10; i++) {
            //     let buttonHtml = `<div class="CounterrOuterrbtn">
            //                         <span class="showNobtn" id="showNobtn${i}">0</span>
            ...
            //     // Get the button ID and the corresponding span ID
            //     let buttonId = $(this).attr('id');
            //     let buttonIndex = buttonId.replace('myButton', '');  // Extract index from ID
            //     let spanId = `#showNobtn${buttonIndex}`;
                
            //     // Get the current value from the span, increment it, and update the span
            //     let currentValue = parseInt($(spanId).text());
            //     let newValue = currentValue;
            //     $(spanId).text(newValue);
            
            //     // Store the updated value in localStorage
            //     localStorage.setItem(`shownobtn${buttonIndex}`, newValue);
            // });
            
            // // On page load, retrieve and set values from localStorage
            // $(document).ready(function () {
            //     for (let i = 0; i < 10; i++) {
            //         let storedValue = localStorage.getItem(`shownobtn${i}`);
            //         if (storedValue !== null) {
            //             $(`#showNobtn${i}`).text(storedValue);
            //         }
            //     }
            // });
...
function spinner_spin_old() {
    ...
    localStorage.setItem('main_score', 0);
        ...
        function storeValues() {
            for (let i = 0; i <= 9; i++) {
                let value = $(`#number${i}`).val();
                localStorage.setItem(`number${i}`, value);
            }
        ...
        function retrieveValues() {
            for (let i = 0; i <= 9; i++) {
                let storedValue = localStorage.getItem(`number${i}`);
                if (storedValue !== null) {
                    $(`#number${i}`).val(storedValue);
                    $(`#showNobtn${i}`).text(storedValue);
                } else {
                    // Optionally handle the case where there is no stored value
                    $(`#number${i}`).val(0);
                    $(`#showNobtn${i}`).text(0);
                }
            }

            let storedTotalNumberPlay = localStorage.getItem('total_number_play');
            if (storedTotalNumberPlay !== null) {
                $("#total_number_play").val(storedTotalNumberPlay);
                $('#totalamt').text(storedTotalNumberPlay);
            } else {
                // Optionally handle the case where there is no stored total number play
                $("#total_number_play").val(0);
                $('#totalamt').text(0);
            }
        }
...
function bet_previous() {
    // Hide the Prev button when clicked
    ...
    $.ajax({
        ...
        success: function(data) {
            if (data) {
                ...
                // Show the Prev button again after processing data
                $('#prevok').hide();
            } else {
                console.log('No data received');
                // Show the Prev button again in case of no data
                $('#prevok').show();
            }
        },
        error: function(xhr, status, error) {
...
function enableButtons()
{
    ...
    // $("#prevok").show();  // Show the Prev button
    // $("#betok").hide();   // Hide the Bet Ok button
    document.getElementById('change_content').innerText = 'Better luck next time! Press Prev to try again.';
    } 
