...
function fetchPlayers() {
    $.ajax({
        url: 'get_all_players_sum_and_values', // Replace with your endpoint URL
        type: 'GET',
        dataType: 'json',
        success: function(data) {
            if (data.players_list) {
                var playersList = data.players_list;
                var $tbody = $('#players_value');
                $tbody.empty(); // Clear the current content
                $.each(playersList, function(index, player) { // Corrected this line
                    var player_url = 'superadmin/players_score_details/' + player.player_id; // Corrected this line
                    var row = '<tr>' +
                        '<td><a href="' + player_url + '">' + htmlspecialchars(player.player_name) + '</a></td>' +
                        '<td>' + htmlspecialchars(player.button_1_value) + '</td>' +
                        '<td>' + htmlspecialchars(player.button_2_value) + '</td>' +
                        '<td>' + htmlspecialchars(player.button_3_value) + '</td>' +
                        '<td>' + htmlspecialchars(player.button_4_value) + '</td>' +
                        '<td>' + htmlspecialchars(player.button_5_value) + '</td>' +
                        '<td>' + htmlspecialchars(player.button_6_value) + '</td>' +
                        '<td>' + htmlspecialchars(player.button_7_value) + '</td>' +
                        '<td>' + htmlspecialchars(player.button_8_value) + '</td>' +
                        '<td>' + htmlspecialchars(player.button_9_value) + '</td>' +
                        '<td>' + htmlspecialchars(player.button_0_value) + '</td>' +
                        '</tr>';
                    $tbody.append(row);
                });
            } else {
            ...
            if (data.sums) {
                var sums = data.sums;
                var $thead = $('#players_sum');
                $thead.empty(); // Clear the current content
                var sumRow = '<tr id="players_playing"><td>Sum</td>';
                var order = [1, 2, 3, 4, 5, 6, 7, 8, 9, 0];
                $.each(order, function(index, key) {
                    sumRow += '<td><b>' + htmlspecialchars(sums[key]) + '</b></td>';
                });
                sumRow += '</tr>';
                $thead.append(sumRow);
            } else {
                console.error('Error fetching sums:', data.error);
            }
        },
