    ...
    else
{
    ...
    $current_data = $this->SuperAdminModel->get_data_with_timestamp();
    
    // Set mode to jackpot_2x
    $data_insert = array(
        'low'          => @$low,
    ...
    $this->SuperAdminModel->number_insert($data_insert);
    ...
    // Prepare updated data after 1 minute
    $updated_data = array(
        'mode' => 'previous_mode', // Replace with the actual previous mode or data you want to revert to
        // Include other fields if needed
    );

    // Update all rows with the new data
    $this->SuperAdminModel->update_all_rows($updated_data);

    // Optionally, update the session mode if needed
    $this->session->set('mode', 'previous_mode');
    // shweta changes end
